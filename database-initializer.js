...
mc.connect("mongodb://127.0.0.1:27017/", { useNewUrlParser: true }, function(err, client) {
	...
	let db = client.db('artworks');  
	
    //Drop the families collection (only if it exists) to start with fresh data.
    //Usually data initialization for start a server or testing.
    db.dropCollection("art",function(err, delOK){
        if(err){
			console.log("Error dropping collection. Likely case: collection did not exist (don't worry unless you get other errors...)")
		}else{
				console.log("Cleared bedrock families collection.");
		}

        //Insert the bedrockFamilies array of object to the database.
		//Each object will be a new document in the families collection.
		//Each document will have an associated _id: ObjectId("SOME_NUMER").
		//Mongo creates an _id for each new document added to the families 
		//collecton.
        db.collection("art").insertMany(artoworks, function(err,result){
            if(err) throw err;
            console.log("Adding the bedrock families to the database.")
            console.log(result);
